...
describe IIIFClient do
  ...
  let(:fixture_path) do
    File.expand_path(File.join(File.dirname(__FILE__), '..', 'fixtures', 'example_iiif_manifest.json'))
  end

  let(:manifest_body) { File.read(fixture_path) }

  describe '#fetch_manifest' do
    it 'parses a v3 manifest and extracts metadata and renderings' do
      response = IIIFClient::HTTPResponse.new('200', { 'content-type' => ['application/json'] }, manifest_body)
